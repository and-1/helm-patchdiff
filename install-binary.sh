...
PROJECT_NAME="helm-patchdiff"
PROJECT_GH="and-1/$PROJECT_NAME"
export GREP_COLOR="never"

HELM_MAJOR_VERSION=$(helm version --client --short | awk -F '.' '{print $1}')

: ${HELM_PLUGIN_DIR:="$(helm env | awk -F\= '/HELM_PLUGINS/{print $2}' | tr -d \")/helm-patchdiff"}
...
getDownloadURL() {
  ...
  if [ -n "$version" ]; then
    DOWNLOAD_URL="https://github.com/$PROJECT_GH/releases/download/$version/helmdiff-$OS.tgz"
  else
...
installFile() {
  ...
  HELM_TMP_BIN="$HELM_TMP/bin/helmdiff"
...
fail_trap() {
  ...
  if [ "$result" != "0" ]; then
    ...
    printf '\tFor support, go to https://github.com/and-1/helm-patchdiff.\n'
  fi
  exit $result
}
...
testVersion() {
  ...
  "${HELM_PLUGIN_DIR}/bin/helmdiff" -h
  set -e
}
